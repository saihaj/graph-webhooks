...
import { CONTOUR_CHART } from "./services/reverse-proxy";
...
async function generateJsonSchemaFromHelmValues(input: string) {
  ...
  return await readFile(jsonSchemaTempFile.name, "utf-8").then((r) =>
    JSON.parse(r),
  );
}
...
async function generateVectorDevTypes() {
  ...
  const jsonSchema = await generateJsonSchemaFromHelmValues(
    valuesTempFile.name,
  );
  const output = await compile(jsonSchema, "VectorValues", {
    additionalProperties: false,
  });
...
async function generateContourTypes() {
  let helmManifest = await fetch(
    `${CONTOUR_CHART.fetchOpts!["repo"]}/index.yaml`,
    {
      redirect: "follow",
    },
  )
  ...
  let relevantChart = helmManifest.entries[
    CONTOUR_CHART["chart"] as string
  ].find((entry) => entry.version === CONTOUR_CHART.version);
